...
def build_user_nav_menu(choice = None):
    navmenus = [
                {'uri': '/my/edit', 'txt':gettext('Change profile')},
                {'uri': '/my/newpwd', 'txt':gettext('Change Password')}]
...
def build_user_left_menu(choice = None):
    leftmenu1 = [
            {'uri': '/my/edit', 'txt':gettext('Change profile')},
            {'uri': '/my/newpwd', 'txt':gettext('Change Password')}]
    leftmenu2 = [
            {'uri': '/my', 'txt':gettext('My Project')}]
    leftmenus={
        gettext('my setting'):leftmenu1,
        gettext('my project'):leftmenu2
    }
    if choice is not None:
        for k,v in leftmenus.items():
            for menu in v:
                if menu['uri'].endswith(choice):
                    menu['choice'] = True
    return leftmenus
...
def view_user(request, name=None):    
    ...
    if request.user == rc.current_user:
        rc.leftmenus=build_user_left_menu()
        return send_response(request, 'user/my.html')
    else:
...
def edit_my(request):
    ...
    rc.leftmenus=build_user_left_menu('edit')
