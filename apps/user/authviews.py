...
from apps.user.views import build_user_left_menu
...
def change_password(request):
    rc = request.rc
    rc.pagename = gettext("Change password")
    rc.navmenus = build_user_nav_menu('newpwd')
    rc.leftmenus=build_user_left_menu('newpwd')
...
def reset_password(request):
    rc = request.rc
    rc.pagename = gettext("Reset password")
    if request.method != 'POST':
