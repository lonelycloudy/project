...
from apps.user.views import build_user_nav_menu
...
def index(request):
    ...
    if ev is None:
        rc.new_projects = Project.objects.filter(status=consts.PROJECT_ENABLE,
                                                 is_public=True).order_by('-ctime')[:5]

        hps = Activity.objects.filter(project__status=consts.PROJECT_ENABLE,
                                      project__is_public=True).values('project').annotate(pc = Count("project")).order_by('-pc')[:5]
        ...
        rc.feature_prjs = FeatureProject.objects.all().order_by('-mtime')[:6]
        rc.logs = get_user_activitys(None, request, 25, False)
        ev = {}
